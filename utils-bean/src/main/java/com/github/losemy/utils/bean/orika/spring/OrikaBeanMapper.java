...
public class OrikaBeanMapper extends ConfigurableMapper implements ApplicationContextAware {
    ...
    private void addAllSpringBeans(ApplicationContext applicationContext) {
        ...
        Iterator mappersIterator = mappers.values().iterator();

        while(mappersIterator.hasNext()) {
            Mapper mapper = (Mapper)mappersIterator.next();
            this.addMapper(mapper);
        }

        Map<String, Converter> converters = applicationContext.getBeansOfType(Converter.class);
        Iterator convertersIterator = converters.values().iterator();

        while(convertersIterator.hasNext()) {
            Converter converter = (Converter)convertersIterator.next();
